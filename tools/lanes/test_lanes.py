...
class OnlyOneDirectionFailsTheRun(unittest.TestCase):
    ...
    def test_a_class_between_the_two_thresholds_is_neither(self):
        # Above `CHEAP` and below `THRESHOLD`: tagged or not, it is nobody's drift, which is what
        # keeps a class hovering either side of the rule from being reported every run.

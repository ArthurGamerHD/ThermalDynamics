// Runs performance benchmarks for Thermal Dynamics implementations and outputs results as JSON.
...
if(args.Contains("--occlusion"))
{
    ...
    int kept=output.Count;
     // Runs the tree-based visibility calculation 100 times and returns the total visible count.
     ...
     // Runs the flat visibility calculation 100 times and returns the total visible count.
...
if(args.Contains("--nearest"))
{
    ...
    foreach(var p in points)if(old.Sample(p,293)!=current.Sample(p,293))throw new Exception("Nearest heat changed");
     // Computes the sum of samples for all points using either the old or new implementation.
...
if(args.Contains("--caps"))
{
    ...
    foreach(var input in inputs)
    {
     ...
     // Computes the total number of cap triangles generated by either the old or new implementation.
    ...
    // Set up test configurations for dense, hull, sparse, multi-cell, and large fixtures.
...
foreach(string name in new[]{"dense","hull","sparse","multi-cell","dense-large"})
{
    ...
    var bounds=new Region(Vector3D.Zero,boundMax,0);
     // Creates the old implementation with the given blocks.
     Old MakeOld() { var f=new Old(1,.6,bounds); foreach(var b in blocks)f.Add(b); return f; }
     // Creates the new implementation with the given blocks.
    ...
    if(maxError!=0) throw new Exception(name+" changed samples: "+maxError);
     // Computes the sum of samples for all points using the old implementation.
     double QueryOld(){double sum=0;foreach(var p in points)sum+=old.Sample(p,293);return sum;}
     // Computes the sum of samples for all points using the new implementation.
     double QueryNew(){double sum=0;foreach(var p in points)sum+=current.Sample(p,293);return sum;}
         // Computes the sum of samples for all points using the new implementation with caching.
    ...
    var oldQuery=Measure(QueryOld);var newQuery=Measure(QueryNew);var cached=Measure(QueryCached);
     // Computes the sum of samples for all points using the new implementation with per-sample caching.
    ...
    // Perform differential checks with varying blur radii, density selection, and block sizes.
...
for(int trial=0;trial<24;trial++)
{
    ...
    // Compare patch output against reference implementation for irregular boxes.
...
if(maxPatchError>.001)throw new Exception("patch temperatures changed");
     // Computes the sum of minimum values across all surface fields using the old implementation.
     ...
     // Computes the sum of minimum values across all surface fields using the new implementation.
...
foreach(var eye in new[]{new Vector3D(10,10,10),new Vector3D(10,10,40),new Vector3D(10,10,100)})
{
    ...
    // Compare celestial sampling performance between baseline and optimized implementations.
...
var skyAngles=Enumerable.Range(0,97).Select(i=>new Vector2D(Math.Cos(i*Math.PI/48),Math.Sin(i*Math.PI/48))).ToArray();
     // Computes the sum of ray.X + normal.Y for all celestial samples using the baseline method.
     double SkyBaseline()
     {
         double sum=0;
     ...
     // Computes the sum of ray.X + normal.Y for all celestial samples using the prepared method.
     double SkyPrepared()
     {
         double sum=0;
...
for(int i=0;i<capCameras.Length;i++)
{
    ...
    if(!expected.SequenceEqual(actual))throw new Exception("Near-plane cap geometry changed");
     ...
     // Computes the total number of vertices in near-plane caps across all cameras using the old implementation.
     ...
     // Computes the total number of vertices in near-plane caps across all cameras using the new implementation with reused arrays.
...
rows.Add(new{name="near-plane-4096-caps",baseline=Measure(OldCaps),optimized=Measure(ReusedCaps)});
    // Compare enumerated and indexed face traversal implementations.
...
foreach(var item in surfaces)
{
    ...
    Array.Copy(item.values,field.Values,item.values.Length);field.BuildFaces(20);drawFields.Add(field);
     ...
     // Computes the sum of patch.Temperatures.X for all faces using enumerated traversal.
     ...
     // Computes the sum of patch.Temperatures.X for all faces using indexed traversal.
     double IndexedFaces()
     {
         double sum=0;
         for(int repeat=0;repeat<16;repeat++) foreach(var field in drawFields)
...
rows.Add(new{name="face-traversal",faces=16*6*drawFields.Count,baseline=Measure(EnumeratedFaces),optimized=Measure(IndexedFaces)});
    // Sample lattice points repeatedly for transition and near-cap preparation.
...
for(int f=0;f<drawFields.Count;f++)
{
    ...
    {
        // Includes clamped points beyond all six faces and exact lattice vertices.
    ...
    latticePoints.Add(points);
     ...
     // Computes the sum of lattice point samples across all fields using the old implementation.
     ...
     // Computes the sum of lattice point samples across all fields using the new implementation.
...
foreach(var item in surfaces)
{
    ...
    for(int axis=0;axis<3;axis++)for(int side=0;side<2;side++)
    {
     ...
     // Computes the sum of minimum values across all surface fields using the eager (baseline) surface build implementation.
...
// Counts the number of patches in a surface field.
...
foreach(var item in surfaces)
{
    ...
    splitCount+=oldCount;plannedCount+=newCount;
     ...
     // Computes the sum of minimum values across all surface fields using the planned (optimized) surface build implementation.
...
Console.WriteLine(JsonSerializer.Serialize(rows,new JsonSerializerOptions{WriteIndented=true}));
    // Measures function performance: median time, allocated bytes, and checksum over multiple runs.

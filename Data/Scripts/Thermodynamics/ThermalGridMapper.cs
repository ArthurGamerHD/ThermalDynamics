...
namespace Thermodynamics
{
    public partial class ThermalGrid : MyGameLogicComponent
    {
        ...
        /// <summary>
        /// The door state handler attached to each door block, kept so it can be detached again.
        /// Subscribing from OnBlockAddOrUpdate without this doubles the handler count on every
        /// door cycle, because the handler re-enters the method that subscribes it.
        /// </summary>
        private Dictionary<long, Action<bool>> DoorStateHandlers = new Dictionary<long, Action<bool>>();
        ...
        {
            IMyDoor removedDoor = block.FatBlock as IMyDoor;
            if (removedDoor != null)
            {
                Action<bool> handler;
                if (DoorStateHandlers.TryGetValue(removedDoor.EntityId, out handler))
                {
                    removedDoor.DoorStateChanged -= handler;
                    DoorStateHandlers.Remove(removedDoor.EntityId);
                }
            }

...
{
    /// <summary>Nested distance bands collected in one source pass; nearby detail has an independent budget.</summary>
    public sealed class ThermalVisionRegionLod : IDisposable
    {
        ...
        // Outer to inner. Even completely filled bands can coarsen without stealing another band's budget.
        ...
        /// <summary>Spend the fine budget on a sampled surface, with angular detail scaling by distance.</summary>
        ...
        public int Advance(int work)
        {
            ...
            // Fine detail first; no publication until the complete set is ready.
        ...
        public bool TryBuild(ThermalVisionRegionScan coarse, int limit, out ThermalVisionRegionOrder order)
        {
            ...
            // Try full LOD first, then sacrifice outer bands before close detail.

...
namespace Thermodynamics.Presentation
{
    ...
    public sealed class ThermalVisionSurfaceField
    {
        ...
        public void BuildFaces(float errorKelvin, bool minimizePatches = false, ThermalVisionSurfaceField previous = null, float leafErrorKelvin = 0)
        {
            ...
            // At most 100 nonempty rectangles per 4x4 face. Reuse the state buffers
            // across six faces; costs count patches, never relax temperature tolerance.
            ...
            for(int axis=0;axis<3;axis++) for(int side=0;side<2;side++)
            {
                ...
                // Flat and nearly unsimplifiable faces cannot justify the extra search.
        ...
        // Refine only single lattice cells. Merged patches may intentionally represent
        // a crease across several cells; replacing those with a bilinear patch would blur it.
        ...
        {
            // A shared partition must represent both endpoints of the fade. A convex
            // temporal blend then retains their error bounds at the checked samples.
        ...
        private void SplitFace(int axis,bool upper,int u0,int u1,int v0,int v1,float error,List<Patch> list)
        {
            ...
            {
                // Parent candidates that split never reach the renderer. Only construct
                // positions for retained leaf patches after the temperature error checks.
        ...
        /// <summary>Approximate the near-camera closing surface using the same scalar field.
        /// Bounded to 64 triangles per input triangle; sampled error is not a global bound.</summary>
        ...
        /// <summary>Exact uniform endpoints permit one constant-colour quad throughout a fade.
        /// This is eligibility accounting only; native submission still uses two triangles.</summary>

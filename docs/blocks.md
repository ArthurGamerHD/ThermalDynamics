...
`/thermal vision`, or the button on any block's terminal, draws a heat-coloured billboard over every
block of the grid you are looking at and the grid you are standing on, out to `ThermalVisionRange`.
`/thermal greyscale` switches between the heat ramp and a linear black-to-white ramp between 250 K
and 900 K. It is client side and draws only; nothing is written to the grid, which is what separates
it from `DebugTemperatureBlockColors`.

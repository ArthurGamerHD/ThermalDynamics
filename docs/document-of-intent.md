...
**Checked by** `C5` (`CoreIsolationTests`). The suite is 1,758 tests, 33 deterministic scenarios and
...
| **The 2–5 minute significance window against `HeatTimeScale` 225.** The balance target asks for the most significant thermal event to land in a 2–5 minute window. **No block in the game lands in it** at the shipped pair, and none can be made to at 225 by the clock alone: of the 72 block types that cannot cool themselves, 0 fall in 120–300 s. | **Resolved 2026-08-23: the window is reachable, and the price it was being weighed against was measured in the wrong environment.** The paired sweep found conductivity ×4 with `HeatTimeScale` 80–120 puts the median hull's crossing at 124–186 s and still brings it back inside the hour, keeping `G1`, `G2` and `G5`, for 1.36–2.04× the shipped substep demand *in vacuum* — where demand is 12 % of what the caps grant. Priced in air, where it is 115 % of them, every one of those cells is **cheaper** than what ships and the shipped pair is the only one that fails `G6`. So the retune is the fix for a defect rather than a cost, and what is left is the pass that carries it: [backlog.md](backlog.md) `C12`. |

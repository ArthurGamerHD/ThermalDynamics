...
| 2026-08-24 | **`G6`'s cost half is scored for the first time and holds.** The criterion has always read *substep demand and step cost*, and the corpus carries no timing column on purpose, so the cost is stated as work — a step's element visits against the 2,000,000 `MaxElementVisitsPerStep` grants, which is the point past which a grid's simulated time runs slower than real time. p99 is 883,675 over 40,656 runs, and the 28 ships past it are 74,000 blocks and up ([backlog.md](backlog.md) `C23`). The figure this row first carried, 446,707, was scored in the unit a step is *paced* in rather than the one the allowance is spent in — corrected 2026-08-24. |

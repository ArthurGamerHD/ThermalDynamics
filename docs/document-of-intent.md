...
| Date | Change |
| --- | --- |
| 2026-08-24 | **`G6`'s cost half is scored for the first time and holds.** The criterion has always read *substep demand and step cost*, and the corpus carries no timing column on purpose, so the cost is stated as work — a step's element visits against the 2,000,000 `MaxElementVisitsPerStep` grants, which is the point past which a grid's simulated time runs slower than real time. p99 is 446,707 over 40,660 runs, and the three ships past it are 350,000 blocks and up ([backlog.md](backlog.md) `C23`). |

...
| `EnableDrag` | 1 | Take the drag out of the ship's motion as well as putting it into the hull. **On since 2026-09-09.** It was off because two mods that both slow a ship down is a collision this mod answers with a switch rather than a detection ([backlog.md](backlog.md) `B38`, `C7`) — that argument still holds and is now a reason to turn it *off*: a world running [RelativeTopSpeed](https://github.com/Gauge/RelativeTopSpeed) alongside this one wants one or the other, because RTS's retarding force exists to stand in for the drag this computes. This mod ships its own version of RTS beside it (`EnableTopSpeed`), so the pairing that needs a decision is now inside one mod and both halves are documented together under [Top speed](#top-speed). |
| `DragCoefficient` | 1.54 | The drag coefficient a hull is treated as having. **It is 1.54 because `EnableShapeDrag` is on, and the two move together or neither should move.** The shape term multiplies the projected area by a `sin²θ` whose population median is **0.3250**, and `0.5 / 0.325 ≈ 1.54` restores what the coefficient was measured to deliver before that term existed; re-scored on the population at this pairing the drag milestone's criterion reads **3.42 % and 78.2 m/s** against the 5 % and 60 m/s registered before the walk, and the cruise altitude medians reproduce exactly ([backlog.md](backlog.md) `K22`). **Turning the shape term off without putting this back to 0.5 makes every hull three times draggier**, passes every test, and reads as a mysteriously unflyable world. It was 0.5 until 2026-09-09, when half was where physics put it: a Newtonian flat-plate projection with no wake and no pressure recovery over-predicts a real bluff body, so the coefficient that matched sat below the one an aerodynamicist would quote — with the shape term carrying that correction instead, this rises past 1 toward the bluff body's own figure. **A different number from `FrictionScale`**: the two are one product and authoring both leaves the share of drag work landing in the surface derived rather than assumed, so a world that tuned its heat has not tuned its handling. |
| `EnableShapeDrag` | 1 | Correct the projected area by the hull's own shape. **The projection is blind by construction** — the friction sum weights each exposed face by `max(0, n_f · ŵ)` over six axis normals, so a stair-stepped 45° slope reads as the flat plate it projects onto ([thermal-model.md](thermal-model.md#the-shape-term-and-why-it-cannot-be-improved-in-place), [backlog.md](backlog.md) `K22`). This gives each block an effective normal read from the cells around it and applies the Newtonian `sin²θ` the projected area is missing. **Measured on the pair that motivated it**: a four-cell brick and a stair-stepped wedge of one frontal cross-section read **172,800 W each** with this off and **100,800 W against 82,215 W** with it on — a ratio of 0.816 where there was none. **On since 2026-09-09**, with `DragCoefficient` raised to 1.54 in the same move to hold the force where it was measured; it was off because it moves temperatures as well as handling — the friction watts it scales are what warm the hull — and `K9`'s rule is that a feature changing the shipped answer is not an addition, so it waited for the population re-walk rather than an argument. **It can only reduce** — the factor is `sin²θ`, at most one — so it lowers heating and drag or leaves them alone, and the heat side is a median **4.56 K cooler** at `reentry` over 500 ships, 496 cooler and none hotter. **One shape reads badly**: a *bare* plate one cell thick is symmetric across its own thin axis, so the reconstruction finds no normal along the flow and charges it 0.111 where the physics says 1 — see [backlog.md](backlog.md) `K24`. Hull skins are not this case; the ship behind them breaks the symmetry. |
| `EnableLift` | 1 | Apply the half of the aerodynamic force that acts *across* the flow. **Not a second model**: Newtonian pressure acts along `−n̂` with magnitude `2q(n̂·ŵ)²dA`, the solver already sums that magnitude, and `ShapeNormal.Factor` collapses it to a scalar — keeping only what points along the flow. Lift is the transverse remainder, so it needs no coefficient to exist ([backlog.md](backlog.md) `K23`). **Needs `EnableDrag` AND `EnableShapeDrag`, both**: the lift force is summed and applied inside the drag pass, which `EnableDrag` gates entirely — this switch alone applies nothing — and without a reconstructed normal every surface is one of six axis planes and the transverse sum describes how a hull was drawn rather than what shape it is. The aero debug view (`DebugAeroOverlay`, `/thermal aero`) names whichever gate is shut. **It adds a force and leaves drag bit-identical**, so a world can take lift without re-tuning the handling it had. **Small on real ships**: over 8,137 published hulls the median lift-to-drag is **0.057** and p95 **0.148**, and lift never exceeds a hull's own weight on any of the 5,649 that can lift themselves — a ship symmetric about its flight axis cancels most of the transverse sum, which is the same reason a cube makes none at all. |
...
The surface cue uses HDR soft depth-tested patches with grazing-angle and distance fades, plus
...
and 32-light caps, are documented in [thermal-glow.md](thermal-glow.md). `/thermal status` reports
hot cues, exposed/in-range blocks visited, submitted glow quads and active lights for diagnosis.
...
| Setting | Default | Effect |
| --- | --- | --- |
| `EnableTopSpeed` | `true` | The master switch. On, the world's cap becomes `SpeedLimit` and every grid is held under its own cruise speed. Off puts the world's own cap back, so a world that stops using this stops flying differently. |
...
| `EnableSpeedBoost` | `true` | Whether thrust may push a ship past its cruise speed at all. On, a ship may reach `*GridMaxBoostSpeed` and is dragged back toward cruise, which is what makes a burst of thrust worth something. Off, the ceiling *is* the cruise speed and a ship cannot pass it. |
...
| `LargeGridMaxBoostSpeed` | 140 m/s | The fastest a boosting large grid may travel, while `EnableSpeedBoost` is on. Never below the grid's own cruise speed, and above `SpeedLimit` it is inert, because the engine's cap is applied first. |
...
| `SmallGridMaxBoostSpeed` | 140 m/s | The same for a small grid. |
...
| Date | Change |
| --- | --- |
| 2026-09-20 | Add HDR glow emission and runtime glow counters to `/thermal status`. |
...
| 2026-09-09 | **The aerodynamic model ships on: `EnableDrag`, `EnableShapeDrag`, `EnableLift` and `EnableTopSpeed` are all on by default, and `DragCoefficient` moves 0.5 → 1.54 with them.** Every one of them was built, measured and scored against a criterion registered before its walk, and every one was left off pending a decision rather than evidence; the decision is taken. **The coefficient is not a separate change** — the shape term multiplies the projected area by a `sin²θ` whose population median is 0.3250, so 0.5 would have made every ship in the game three times slipperier than the coefficient was ever measured to make it, and 1.54 is what holds the population where `K22` scored it (3.42 % and 78.2 m/s against a 5 % and 60 m/s criterion). **What a player will notice**, in order: the world speed cap becomes `SpeedLimit` 140 m/s and each ship gets a cruise speed from its own mass; air slows ships down and pushes them sideways a little; and hulls run a median **4.56 K cooler** in atmosphere than they did, because the shape term takes friction off as well as drag. `EnableWindwardShielding` stays **off** — it was not part of the configuration `K22` scored, and shipping it on would ship a population figure nobody has measured. The one shape the term reads badly is a bare one-cell plate ([backlog.md](backlog.md) `K24`). |
| 2026-09-09 | **Two defects in the top-speed port, both found by the aero wiring trace on 2026-09-02 and both fixed.** `EnableSpeedBoost` was **inert**: the switch was read one line after a `speed <= cruise` return had already made its condition unreachable, so all three documented rungs behaved as the third and no world ever got the hard cap at cruise. What it decides is the ceiling handed to the engine, and that choice now lives in `TopSpeedForce.Ceiling` where `TopSpeedForceTests` can ask what the switch changes — `ThermalGridTopSpeed` needs a session, a physics component and a grid group, which is why nothing caught it. And `LargeGridMaxBoostSpeed` / `SmallGridMaxBoostSpeed` were documented and labelled as **newtons**: the value is `AddForce`'s `maxSpeed`, which clamps a body's velocity, so they are **m/s** and always were. The numbers did not move; what they are called did. |

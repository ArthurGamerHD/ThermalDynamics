...
looks like — so brightness and colour are both functions of temperature alone and are the same on
every block in the game: nothing below the Draper point at 798 K, dull red just above it, orange at
1,500 K and above. It is deliberately *not* a share of each block's rating: that reads as a fuel
gauge for failure rather than as a hot object, it repeats what the engine's own damage effects
already show, and it costs the property that makes the channel worth having — that what a player
learns on one block transfers to every other. Nothing the game ships glows from the weather; the
hottest planet in `Planets.xml` runs a 390 K day, which is four hundred degrees short. See

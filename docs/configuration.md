...
| `EnableDrag` | 0 | Take the drag out of the ship's motion as well as putting it into the hull. **Off**, because two mods that both slow a ship down is a collision this mod answers with a switch rather than a detection ([backlog.md](backlog.md) `B38`, `C7`). |
| `DragCoefficient` | 1 | The drag coefficient a hull is treated as having. A bluff body's, because a Space Engineers hull is a brick. **A different number from `FrictionScale`**: the two are one product and authoring both leaves the share of drag work landing in the surface derived rather than assumed, so a world that tuned its heat has not tuned its handling. |

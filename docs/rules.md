...
**And the test tree scans itself now, which it did not until 2026-08-27.** This entry used to end
by naming that gap, and closing it found two more on the first run: `Scenarios.OpenSides`, a
documented six-face count nothing called, and `IncandescenceTests.Luminance`, a photopic integration
written from first principles and then never run — which took `Photopic` with it. The compiler
cannot do this: an unused private *field* is a warning and an unused private *method* is not, so
these compile, get maintained, and read to the next person as something the suite relies on.
...
`UncalledCodeTests.NoHelperInTheTestTreeIsCalledByNothing` for the test tree's own helpers. That
last one counts identifiers across `Data/` and `tests/`, so an *overload* of a name something else

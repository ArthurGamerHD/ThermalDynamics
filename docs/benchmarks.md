...
| 19 | 2026-09-04 | *(the tenth performance pass, tip)* | 2,260 | **1 m 12 s** | +178 across the pass and the cleanup effort between: `RoomMapRecyclingTests`, `StepPrologueTests`, `FirstStepLabTests`, four bitset pins in `SunShadowMapTests`, and the cleanup effort's own suites. **Most of this pass's wins are per-event, so no solver row moved**: the first step after a rebuild, the first sunlit step, a sun-drift rebuild and a steady room remap all stopped paying costs the steady step never paid ([performance.md](performance.md#pass-10--what-the-pass-moved)). **The committed baseline was re-taken this day, and the comparison that re-took it is not evidence of anything**: the old baseline was recorded 2026-08-22 on the unoptimised build (`M13`) and a census hull two refreshes old (`M11`), so its −50 to −74 % step rows mix three eras, and four sub-millisecond rows read +200–540 % from inside the old noise floor. The first comparison against the *new* baseline is the first one since 2026-08-22 that will mean anything. |
...
| Date | Change |
| --- | --- |
| 2026-09-04 | **The committed baseline is re-taken — it had been the 2026-08-22 unoptimised-build baseline for the whole optimised era.** Every `bench report --baseline` comparison since 2026-08-26 crossed the `M13` era boundary and a census refresh (`M11`), which is exactly the comparison `M7` forbids; nothing failed because the baseline tests pin the key contract (`M6`), not the values. Iteration-log row 19 carries pass 10. |

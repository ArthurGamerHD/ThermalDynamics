...
| B8 | **The settings surface is being rebuilt**, and the plan has its own document: a page per system, definition files brought into the menu, Status as a real report, the config file regrouped, and per-subtype block overrides. Steps 1 and part of 2 are done. | [settings-redesign.md](settings-redesign.md) |
| B9 | **A startup crash is unexplained.** `ThermalGrid.Tick` now catches and logs instead of taking the session down, so the next run names the inner frames. Until then everything built since is standing on a mod that may not start. | — |

...
| D13 | **A grid is visited every frame whether or not the frame does solver work.** On a one-block grid `Update`'s own per-frame cost — the settings check, the dirty branches, the profiler scopes — is 34 % of what the grid costs, and no change to a step can reach it. Only visiting fewer grids per frame can. | [benchmarks.md](benchmarks.md#below-eight-hundred-blocks) |

...
| A5 | The step budget (`MaxLinkVisitsPerStep`) counts link visits but not node visits. **Measured**: a node is worth 3–8 links where the budget binds, an exposed face 0.1–0.5, so counting links alone under-charges a real ship by 2.4–4×. Recommendation is `links + 4·nodes`; the change itself is not yet made. | [element-cost.md](element-cost.md) |

...
| B2 | The coolant pump's terminal on/off switch does nothing. **Established**: this is an unfinished gameplay mechanic, not an artifact — `CoolantPump` reads `Enabled`, `Speed`, `PowerAvailable` and `MaxPowerWatts` from a host that never writes them, `DemandWatts` carries balance reasoning about pump power, and `blocks.md` and `definitions.md` already describe pumps as switchable and powered. `MaxPowerWatts` is never assigned, so pumps would draw 0 W even once wired. Open question is narrower: should a loop need electrical power? | open decision, see B7 |
...
| B7 | **No block has a terminal slider, and two throttles in the model have no way to be set.** `ThermalTerminal.OnCustomControlGetter` hooks `AppendingCustomInfo` and adds no controls at all. `HeatPumpDevice.PowerSetting` and `CoolantPump.Speed` are both 0..1, both read by the power and flow arithmetic, and both written by nothing — so a heat pump always runs at full rating and a coolant pump always at full speed. The terminal already renders `throttled N%` for a figure that cannot change. Same family as B2: the model was built to be driven from a terminal that was never added. A custom slider also needs its own multiplayer sync, so it lands on B4. | [ThermalTerminal.cs](../Data/Scripts/Thermodynamics/ThermalTerminal.cs) |

...
| B2 | ~~The coolant pump's terminal on/off switch does nothing.~~ **Wired** — the switch and speed now reach `CoolantPump.Enabled` / `.Speed`, and the loop's cached flow is refreshed when either moves. **Still open**: whether a loop should draw power. `MaxPowerWatts` is still never assigned, so pumps draw nothing. | open decision |

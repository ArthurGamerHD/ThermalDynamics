...
| B3 | The settings menu still cannot change anything from a multiplayer client. Now unblocked: settings replicate server → client, so the remaining work is a client→server request path with a permission check, rather than replication itself. | [known-issues.md](known-issues.md) |
| B4 | ~~Nothing registered on the network channel.~~ **Partly done** — SENetworkAPI upgraded to 2.0 and three properties registered: the world's settings (server → client, which clients previously did not receive at all) and the two pump throttles. Block *temperatures* are still unreconciled — clients re-simulate and drift — which is the remaining half. | [known-issues.md](known-issues.md) |
